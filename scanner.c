...
Token scanner() {
    ...
    while (true) {
        if (currentChar == '#') {
            // Skip comments
            currentChar = getNextChar(); // Get the next character after '#'
            while (currentChar != '#' && currentChar != EOF) {
                currentChar = getNextChar(); // Continue until another '#' or EOF
            }
            if (currentChar == EOF) {
                lexicalError("Unclosed comment");
            }
            currentChar = getNextChar(); // Move past the closing '#'
        } else if (isspace(currentChar)) {
            currentChar = getNextChar(); // Skip whitespace
        } else {
            break; // Non-comment, non-whitespace character found
        }
    }
